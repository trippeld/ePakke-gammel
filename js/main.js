$(document).ready(function(){
	...
	*/
	...
	$("body").delegate(".selectProduct","click",function(event){
		...
		var pid = $(this).attr('pid');
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{selectProduct:1,product_id:pid},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
	...
	*/
	$("body").delegate(".qty","keyup",function(event){
		...
		$('.net_total').html("Å betale : kr " +net_total);
	})
	...
	*/
	$("body").delegate(".updatep","click",function(event){
		var update = $(this).parent().parent().parent();
		var update_plus = update.find(".updatep").attr("update_plus");
		var qty = update.find(".qty").val();
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{updatePCartItem:1,update_plus:update_plus,qty:qty},
			success	:	function(data){
				$("#cart_msg").html(data);
				count_item();
				checkOutDetails();
	...
	$("body").delegate(".updatem","click",function(event){
		var update = $(this).parent().parent().parent();
		var update_minus = update.find(".updatem").attr("update_minus");
		var qty = update.find(".qty").val();
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{updateMCartItem:1,update_minus:update_minus,qty:qty},
			success	:	function(data){
				$("#cart_msg").html(data);
				count_item();
				checkOutDetails();
